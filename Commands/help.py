...
from Config.colors import white_color, red_color, yellow_color, blurple_color, grey_color, green_color, black_color
...
class DropdownView(disnake.ui.View):
    ...
    async def interaction_check(self, interaction: disnake.Interaction):
        if interaction.author.mention not in interaction.message.embeds[0].description:
            ErrorEmbed = disnake.Embed(title=f"{error_emoji} | Erro!", description=f"Você não pode usar esse painel de comandos! Caso queira ver todos os meus comandos, use ``/comandos``.", timestamp=datetime.datetime.utcnow(), color=red_color)
